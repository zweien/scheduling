...
test('多次交换后每个日期仍保留各自最初值班员', async () => {
  const { swapSchedulesWithReason } = await loadModule();
  const deps = createDependencies();

  deps.schedulesByDate.set('2026-03-19', {
    id: 3,
    date: '2026-03-19',
    user_id: 3,
    original_user_id: 3,
  ...
  const firstSwap = await swapSchedulesWithReason({
    date1: '2026-03-17',
    date2: '2026-03-18',
    reason: '第一次交换白夜班值守说明',
  }, deps);

  assert.equal(firstSwap.success, true);
  assert.equal(deps.schedulesByDate.get('2026-03-17')?.user_id, 2);
  assert.equal(deps.schedulesByDate.get('2026-03-17')?.original_user_id, 1);
  assert.equal(deps.schedulesByDate.get('2026-03-18')?.user_id, 1);
  assert.equal(deps.schedulesByDate.get('2026-03-18')?.original_user_id, 2);

  const secondSwap = await swapSchedulesWithReason({
    date1: '2026-03-18',
    date2: '2026-03-19',
    reason: '第二次交换白夜班值守说明',
  }, deps);

  assert.equal(secondSwap.success, true);
  assert.equal(deps.schedulesByDate.get('2026-03-18')?.user_id, 3);
  assert.equal(deps.schedulesByDate.get('2026-03-18')?.original_user_id, 2);
  assert.equal(deps.schedulesByDate.get('2026-03-19')?.user_id, 1);
  assert.equal(deps.schedulesByDate.get('2026-03-19')?.original_user_id, 3);
});
