...
test('月历视图点击已有排班日期可删除当天值班', async ({ page }) => {
  await login(page);
  ...
  await expect(page.getByRole('button', { name: '删除当天值班' })).toBeVisible();
  await page.getByRole('button', { name: '删除当天值班' }).click();

  await expect(page.getByRole('dialog')).toHaveCount(0);
  await expect(page.getByText('张', { exact: true })).toHaveCount(0);
});
